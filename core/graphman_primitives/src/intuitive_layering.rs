use crate::GraphmanCommand;
use crate::GraphmanLayer;

/// Allows a command to be extended with additional features
...
impl<C> IntuitiveLayering<C, ()> {
    /// Adds intuitive layering support to a command.
    ...
    /// Extends the command with additional features.
    ...
    (@command $l1:ident, $( $lx_wo_l1:ident ),* $(,)* ; ; $( $lx_wo_ln:ident ),* $(,)* ; $ln:ident) => {
            ...
            $ln::Outer: GraphmanCommand<Ctx>
        {
            ...
            fn execute(self, ctx: Ctx) -> Self::Future {
                ...
                let output = $l1.layer(self.command);
