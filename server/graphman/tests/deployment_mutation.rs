...
async fn assert_deployment_paused(hash: &str, should_be_paused: bool) {
    let query = r#"query DeploymentStatus($hash: String!) {
        deployment {
            info(deployment: { ipfs: { hash: $hash } }) {
                status {
                    isPaused
                }
            }
    ...
    run_test(|| async {
        ...
        let resp = send_graphql_request(
            json!({
                "query": r#"mutation {
                    deployment {
                        pause(deployment: { ipfs: { hash: "subgraph_2" } })
                    }
                }"#
            }),
            VALID_TOKEN,
    ...
    run_test(|| async {
        ...
        send_graphql_request(
            json!({
                "query": r#"mutation {
                    deployment {
                        pause(deployment: { ipfs: { hash: "subgraph_1" } })
                    }
                }"#
            }),
            VALID_TOKEN,
        ...
        send_graphql_request(
            json!({
                "query": r#"mutation {
                    deployment {
                        resume(deployment: { ipfs: { hash: "subgraph_1" } })
                    }
                }"#
            }),
            VALID_TOKEN,
    ...
    run_test(|| async {
        ...
        send_graphql_request(
            json!({
                "query": r#"mutation {
                    deployment {
                        restart(deployment: { ipfs: { hash: "subgraph_2" } }, delaySeconds: 2)
                    }
                }"#
            }),
            VALID_TOKEN,
    ...
    run_test(|| async {
        ...
        let resp = send_graphql_request(
            json!({
                "query": r#"mutation {
                    deployment {
                        restart(deployment: { ipfs: { hash: "subgraph_1" } }, delaySeconds: 2)
                    }
                }"#
            }),
            VALID_TOKEN,
        ...
        let query = r#"query TrackRestartDeployment($id: UUID!) {
            execution {
                info(id: $id) {
                    id
                    kind
